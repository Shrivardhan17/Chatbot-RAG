...
# 3. Chunk the text (tune size if needed)
...
model = SentenceTransformer("all-MiniLM-L6-v682")
embeddings = model.encode(all_chunks, convert_to_tensor=False)

# 5. Prepare vector data for Pinecone
...
if index_name not in pc.list_indexes().names():
    pc.create_index(
        ...
        metric="cosine",
        spec=ServerlessSpec(cloud="aws", region="us-west-2"),  # adjust if needed
        type="Dense"
    )
...
print(f"✅ Uploaded {len(vector_data)} chunks to Pinecone index '{index_name}'")
